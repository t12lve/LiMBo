...
// 1. Setup installer
const setupExe = path.join(nsisTarget, "LiMBo_0.3.0_x64-setup.exe");
if (existsSync(setupExe)) {
  cpSync(setupExe, path.join(releaseDir, "LiMBo_0.3.0_x64-setup.exe"));
} else {
  // fallback to prod/desktop/LiMBo-Installer.exe
  cpSync(path.join(repo, "prod", "desktop", "LiMBo-Installer.exe"), path.join(releaseDir, "LiMBo_0.3.0_x64-setup.exe"));
}
...
const portableZip = path.join(releaseDir, "LiMBo-desktop-portable-0.3.0.zip");
...
if (existsSync(crxSrc)) {
  cpSync(crxSrc, path.join(releaseDir, "LiMBo-chrome-0.3.0.crx"));
}

// 4. Extension zip
const extZipSrc = path.join(repo, "prod", "chrome", "LiMBo-chrome.zip");
if (existsSync(extZipSrc)) {
  cpSync(extZipSrc, path.join(releaseDir, "LiMBo-extension-0.3.0.zip"));
}

// 5. Firefox XPI
const ffXpiSrc = path.join(repo, "prod", "firefox", "LiMBo-firefox.xpi");
if (existsSync(ffXpiSrc)) {
  cpSync(ffXpiSrc, path.join(releaseDir, "LiMBo-firefox-0.3.0.xpi"));
}

// 6. Premiere CEP zip
const premiereZip = path.join(releaseDir, "LiMBo-premiere-0.3.0.zip");
